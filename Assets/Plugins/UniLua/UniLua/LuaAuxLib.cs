...
namespace UniLua
{
	...
	public partial class LuaState
	{
        ...
        {
            object obj = API.ToUserData(narg);
            return obj;
        }

        public object[] L_CheckTable(int narg)
        {
            LuaTable tbl = API.ToTable(narg);
            ...
            for (int i = 0; i < tbl.Length; ++i)
            {
                ...
                switch (val.V.Tt)
                {
                    ...
                    case (int)LuaType.LUA_TBOOLEAN:
                        ret.Add(val.V.BValue());
                        break;
                    case (int)LuaType.LUA_TNUMBER:
                        ret.Add(val.V.NValue);
                        break;
                    case (int)LuaType.LUA_TUINT64:
                        ret.Add(val.V.UInt64Value);
                        break;
                    case (int)LuaType.LUA_TSTRING:
                        ret.Add(val.V.SValue());
                        break;
			...
			if( !L_CallMeta( index, "__tostring" ) ) // no metafield? // TODO L_CallMeta
			{
				switch( API.Type(index) )
				{
					...
					case LuaType.LUA_TUINT64:
						API.PushString( string.Format("{0}", API.ToUInt64(index)));
						break;
