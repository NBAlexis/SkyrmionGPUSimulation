...
namespace UniLua
{
	...
	public partial class LuaState : ILuaState
	{
		...
		void ILuaAPI.PushTable(LuaTable tbl)
		{
			Top.V.SetHValue(tbl);
			ApiIncrTop();
		}
        ...
        LuaTable ILuaAPI.ToTable(int index)
        {
            StkId addr;
            if (!Index2Addr(index, out addr))
                return null;

            return addr.V.TtIsTable() ? addr.V.OValue as LuaTable : null;
        }
